...
The graph maps 31 public operations into 10 accountable capabilities and 9 controls. It covers API discovery, Practitioners, Bookings, Sessions, eligibility, Coverage, pre-authorization, Claim valuation, remittance and Webhooks.

<CardGroup cols={2}>
  <Card title="Run the benchmark" icon="flask" href="https://github.com/heyrafiki/proving-ground">
    Reproduce four suites and five adversarial mutation checks from the public manifest.
  </Card>
  <Card title="Open the graph" icon="diagram-project" href="https://github.com/heyrafiki/contract/blob/main/assurance/assurance-graph.json">
    Inspect the machine-readable capability, source, control and evidence links.
  </Card>
  <Card title="Read the schema" icon="brackets-curly" href="https://github.com/heyrafiki/contract/blob/main/assurance/assurance-graph.schema.json">
...
Create a key in the [Developer Platform](https://app.heyrafiki.space/dev/keys) if your Organization has access. Otherwise [request access](https://heyrafiki.space/waitlist) and review the [acceptance test plan](/insurance/acceptance-testing) now.
...
The graph turns each research hypothesis into a reviewable path through stated
assumptions, versioned evidence, baseline methods and reproducible results.
